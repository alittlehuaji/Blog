...
2. Options error（选项错误）
   ...
   ```log
   Jun 14 22:03:03 iZ2vc5dobammin8x5yzvdgZ ovpn-server[7383]: Options error: --status fails with '/etc/openvpn/log/openvpn-status.log': No such file or directory (errno=2)
   Jun 14 22:03:03 iZ2vc5dobammin8x5yzvdgZ ovpn-server[7383]: Options error: --client-config-dir fails with '/etc/openvpn/ccd': No such file or directory (errno=2)
   Jun 14 22:03:03 iZ2vc5dobammin8x5yzvdgZ ovpn-server[7383]: Options error: Please correct these errors.
   Jun 14 22:03:03 iZ2vc5dobammin8x5yzvdgZ ovpn-server[7383]: Use --help for more information.
   Jun 14 22:03:03 iZ2vc5dobammin8x5yzvdgZ systemd[1]: openvpn@server.service: Main process exited, code=exited, status=1/FAILURE
   ```
...
如上文，`No such file or directory`，你可以根据前面的路径来创建文件，比如 `/etc/openvpn/log/openvpn-status.log`这个文件找不到。那么你可以使用 `touch /etc/openvpn/log/openvpn-status.log` 来创建一个
